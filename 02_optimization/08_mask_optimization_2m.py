    ...
    for row in range(depth_in_sink.configs.rows):
        for col in range(depth_in_sink.configs.columns):
            if depth_in_sink[row, col] == depth_in_sink.configs.nodata:
                ...
                volume = depth_in_sink[row, col] * 4
...
plot_figure_path = 'scatter_plot_DEM2m.png'
...
result_df.to_csv('output_solution_DEM2m.csv', index=False)
result_df = pd.DataFrame(X)
result_df.to_csv('output_variable_DEM2m.csv', index=False)
